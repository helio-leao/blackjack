...
export default function App() {
  ...
  const [bet, setBet] = useState<number>(blackJack.credits * INITIAL_BET_FACTOR)
  ...
  return (
    <>
      {/* Title */}
      <h1 style={{margin: '20px 0', color: 'yellow'}}>
        BLACKJACK
      </h1>
      ...
      <div style={{marginTop: '40px', display: 'flex', gap: '5px'}}>
        ...
        />
        <Button
          disabled={status === BlackJack.Status.HIT ||
            status === BlackJack.Status.DEAL}
          onClick={deal}
          text="Deal"
        />
        <Button
      ...
      {playerCards.length > 0 &&
        <Hand
          title={`Player's Hand (${playerPoints})`}
          cards={playerCards}
        />
      }
      ...
      {dealerCards.length > 0 &&
        <Hand
          title={`Dealer's Hand (${dealerPoints})`}
          cards={dealerCards}
        />
      }
    </>
